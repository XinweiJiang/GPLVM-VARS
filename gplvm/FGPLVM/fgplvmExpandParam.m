...
if isfield(model, 'back') & ~isempty(model.back)
  ...
  clear tmp tmp_dim;
    ...
    if isfield(model,'withLinear') && model.withLinear == 1     %for linear bc-gplvm
        endVal = model.d*model.q;
        model.W = reshape(params(startVal:endVal), model.d, model.q);
        model.X = model.y*model.W;
    else        %for original gplvm
        endVal = model.N*model.q;
        model.X = reshape(params(startVal:endVal), model.N, model.q);
    end
    if isfield(model,'oriX_NoLastOne') && ~isempty(model.oriX_NoLastOne)     %update last one only
        model.X(1:model.N-1, :) = model.oriX_NoLastOne;
    end
end
startVal = endVal+1;
...
if isfield(model,'NoOptHyper') && model.NoOptHyper == 1
    model = gpExpandParam(model, model.OptHyper);
    model.kern.hyper = model.OptHyper;
else
