function [ z, retAcc, retAccCell ] = SFgplvm( dataSetName, x, y, xx, yy, latentDim, Opt )
...
options.initX = ppcaEmbed(x,latentDim);
if size(y,2) == 1
...
if ispc
    filename = ['demSFgplvm' dataSetName 'Tr' num2str(size(x, 1)) 'L' num2str(latentDim) strtemp];
    plotZ(model.X, y, filename,isAutoClosePlot);
    filename = ['demSFgplvm' dataSetName 'Te' num2str(size(xx, 1)) 'L' num2str(latentDim) strtemp];
    plotZ(zcInit, yy, filename,isAutoClosePlot);
end

z = model.X;
filename = ['demSFgplvm' dataSetName 'Tr' num2str(size(x, 1)) 'Te' num2str(size(xx, 1)) 'L' num2str(latentDim) strtemp];
save([filename]);
