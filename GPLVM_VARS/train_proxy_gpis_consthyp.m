function [hyp2,datastruc,inp_ind] = train_proxy_gpis_consthyp(input_cmg_mat,out_data,tar_data, cov_id_cell_fcst, bUseGPLVM4GPIS)

% this function is same as train_proxy_gpis.m but it takes additioanl input
...
%      end
    ...
    if bUseGPLVM4GPIS == 0
        ...
        % Xinwei: 使用GPLVM代替GPR，更make sense并提高速度，特别是当待优化的参数数量num_dim较多时 2019.5.10
        %% Set up the GP model(x: lvms; y: parameters)
        for jj = 1 : num_lvm
...
%     %%Train GPLVM where unknown parameters are inputs and LVMs are outputs
%     % Set up GPLVM model
